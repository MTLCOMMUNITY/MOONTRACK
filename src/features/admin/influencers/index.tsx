...
export function AdminInfluencers() {
  ...
  async function load() {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/admin-get-influencers`,
        {
          method: 'POST',
          headers: {
  ...
  async function loadDetails(id: string) {
    ...
    try {
      ...
      const { data: conversions, error: convError } = await supabase
        .from('conversions')
        .select(
          'id, student_name, student_email, registered_at, payment_status, payments(amount)'
        )
        .eq('influencer_id', id)
      ...
      const totalClicks = (linkData ?? []).reduce(
        (sum, l) => sum + (l.click_count ?? 0),
        0
      )
      const totalConversions = conversions?.length ?? 0
      const totalEarnings = (payments ?? []).reduce(
        (sum, p) => sum + (p.commission_earned ?? 0),
        0
      )
      ...
      const conversionRate =
        totalClicks > 0
          ? parseFloat(((totalConversions / totalClicks) * 100).toFixed(1))
          : 0
  ...
  async function handleInvite() {
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/invite-influencer`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  async function toggleStatus(id: string, currentStatus: boolean) {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/toggle-influencer`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      toast.success(
        `Influencer ${!currentStatus ? 'reactivated' : 'suspended'} successfully`
      )
      load()
    } catch (err) {
      const error = err as Error
  ...
  async function handleDelete(id: string, name: string) {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/delete-influencer`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  return (
    <>
        ...
        <div className='ms-auto flex items-center gap-2'>
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button size='sm'>
                <IconPlus className='me-1 size-4' /> Invite Influencer
              </Button>
            </DialogTrigger>
        ...
        <Card>
          <CardHeader>
            <CardTitle className='text-base font-semibold'>
              All Influencers ({influencers.length})
            </CardTitle>
          </CardHeader>
          <CardContent className='p-0'>
            {loading ? (
              ...
              <div className='py-16 text-center text-sm text-muted-foreground'>
                No influencers yet. Invite one above.
              </div>
            ) : (
                      ...
                      >
                        ...
                        <TableCell>
                          {!inf.is_active ? (
                            ...
                            <Badge
                              variant='outline'
                              className='border-yellow-500/20 bg-yellow-500/10 text-yellow-600 hover:bg-yellow-500/20'
                            >
                              Pending (Invite Sent)
                            </Badge>
                          ) : (
                        ...
                        <TableCell onClick={(e) => e.stopPropagation()}>
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align='end'>
                              <DropdownMenuItem
                                onClick={() =>
                                  toggleStatus(inf.id, inf.is_active)
                                }
                                className={
                                  inf.is_active
                                    ? 'text-orange-600'
                              ...
                              >
                                {inf.is_active ? (
                                  <>
                                    <IconBan className='mr-2 h-4 w-4' /> Suspend
                                    Influencer
                                  </>
                                ) : (
                                  <>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>{deleteTarget?.name}</strong> and remove all their
              referral links from the system.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      >
        <DialogContent className='max-h-[95vh] max-w-full overflow-y-auto p-6 sm:max-w-6xl'>
          <DialogHeader>
          ...
          {!detailsLoading && selectedInfluencer && (
            <div className='space-y-6 py-4'>
              {/* Profile Card Header */}
              <div className='flex flex-col gap-4 rounded-xl border border-[#0A0F1E]/10 bg-slate-50/50 p-4 sm:flex-row sm:items-center sm:justify-between dark:border-white/10 dark:bg-white/5'>
                <div className='flex items-center space-x-4'>
                ...
                <div className='flex flex-wrap items-center gap-3'>
                  <div className='flex items-center space-x-2 rounded-lg border bg-white px-3 py-1.5 dark:bg-slate-900'>
                    ...
                    <code className='font-mono text-xs font-bold text-yellow-600 dark:text-yellow-400'>
                      {selectedInfluencer.ref_code}
                    </code>
                    <button
                      onClick={() => {
              ...
              {details && (
                <div className='grid grid-cols-2 gap-4 sm:grid-cols-4'>
                  <Card>
                    ...
                    <CardContent>
                      <div className='text-xl font-bold'>
                        {details.totalClicks.toLocaleString()}
                      </div>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className='text-xl font-bold'>
                        {details.totalConversions.toLocaleString()}
                      </div>
                      <p className='mt-1 text-[10px] text-muted-foreground'>
                  ...
                  <Card className='border-yellow-500/20 bg-yellow-500/[0.02]'>
                    <CardHeader className='flex flex-row items-center justify-between pb-2'>
                      <CardTitle className='text-xs font-medium tracking-wider text-yellow-600 uppercase dark:text-yellow-400'>
                        Pending Bal
                      </CardTitle>
                ...
                <Tabs defaultValue='conversions' className='w-full'>
                  <TabsList className='grid w-full grid-cols-2'>
                    <TabsTrigger value='conversions'>
                      Conversions ({details.conversions.length})
                    </TabsTrigger>
                    <TabsTrigger value='payouts'>
                      Payouts ({details.payouts.length})
                    </TabsTrigger>
                  </TabsList>

                  <TabsContent
                    value='conversions'
                    className='mt-4 overflow-hidden rounded-xl border bg-white dark:bg-slate-900'
                  >
